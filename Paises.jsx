  ...
  const SaveModal = async () => {
    ...
    if (Object.values(newErrors).every((error) => error === "")) {
      try {
        ...
        const response = await axios.get(`http://localhost:3000/pais?N_Pais=${modalValues.pais}`);
        if (response.data.some((pais) => pais.N_Pais.toLowerCase() === modalValues.pais.toLowerCase())) {
        ...
        const insertResponse = await axios.post(`http://localhost:3000/pais`, newPais);
