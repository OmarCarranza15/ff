...
function Usuario() {
  ...
  const SaveModal = async () => {
    const trimmedUsuario = modalValues.Usuario.trim();
    ...
    } else if (!/^[a-zA-ZáéíóúñÁÉÍÓÚÑ\s-]+$/.test(trimmedNombre)) {
      newErrors.Nombre =
      ...
      try {
        ...
        const usuarioExistente = usuarios.some(
          (usuario) =>
            ...
            trimmedUsuario.toLowerCase()
        );

        if (usuarioExistente) {
          const errorMessages = {};
          if (usuarioExistente) {
            errorMessages.Usuario = "El Usuario ya existe";
          }
        ...
        const auditoriaData = {
          Campo_Nuevo: `- Puesto Interno: ${puestoinResponse.N_PuestoIn || "No Disponible"}¬ 
                        - Rol del Usuario: ${rolusuarioResponse.N_Rol || "No Disponible"}¬
                         - Usuario: ${trimmedUsuario}¬ - Nombre: ${trimmedNombre}¬ 
                         - Estado: NUEVO `,
          Tabla: "Usuarios",
          Accion: 1, // Inserción
          ID_Usuario: userId,
...
const saveChanges = async (id) => {
  ...
  try {
    ...
    if (currentRow.Estado === "NUEVO") {
      ...
      const originalPuesto = originalRow.N_PuestoIn;
      ...
      const editedPuesto = editedRow.N_PuestoIn;
      const editedRol = editedRow.N_Rol;
      const editedUsuario = trimmedUsuario;
      ...
      const auditoriaData = {
        Campo_Original: [
          hasPuestoChanged ? `- Puesto Interno: ${originalPuesto}` : '',
          hasRolChanged ? `- Rol del Usuario: ${originalUsuario}` : '',
          hasUsuarioChanged ? `- Usuario: ${originalUsuario}` : '',
          hasNombreChanged ? `- Nombre: ${originalNombre}` : '',
          hasEstadoChanged ? `- Estado: ${originalEstado}`: ''
        ].filter(Boolean).join('¬ '),
        Campo_Nuevo: [
          hasPuestoChanged ? `- Puesto Interno: ${editedPuesto}` : '',
          hasRolChanged ? `- Rol del Usuario: ${editedRol}` : '',
