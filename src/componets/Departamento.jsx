...
function Departamento() {
  ...
  const SaveModal = async () => {
    const trimmedDepartamento = modalValues.departamento.trim();
    ...
    } else if (!/^[a-zA-ZñÑáéíóúÁÉÍÓÚ\s]+$/.test(trimmedDepartamento)) {
      ...
      try {
        ...
        const departamentoExists = response.data.some(
          (departamento) =>
            ...
            trimmedDepartamento.toLowerCase()
        );
        ...
        const newDepartamento = {
          N_Departamento: trimmedDepartamento,
        ...
        const auditoriaData = {
          Campo_Nuevo: `- País: ${paisResponse.data.N_Pais}¬ - Departamento: ${trimmedDepartamento}`,
          Tabla: "Departamentos",
          Accion: 1, // Inserción
          ID_Usuario: userId,
          N: newIndex
  ...
  const saveChanges = async (id) => {
    const trimmedDepartamento = editedRow.N_Departamento.trim();
    if (!trimmedDepartamento) {
      setErrors({ N_Departamento: "El campo Departamento no puede estar vacío" });
      showNotification("El campo Departamento no puede estar vacío");
      return;
    }
    try {
      const updateRow = {
      ...
      if (editedRow && editedRow.ID_Pais && editedRow.N_Departamento) {
        ...
        const departamentoExists = response.data.some((departamento) => {
          if (
            ...
            const editedDivisionName = trimmedDepartamento
              ? trimmedDepartamento.toLowerCase()
              : "";
            return (
      ...
      const editedDepartamento = trimmedDepartamento;
  ...
  const columns = [
    ...
    },
    {
      ...
      maxWidth: "350px", // Ajusta el tamaño máximo según sea necesario
      cell: (row) =>
        editMode && editedRow?.id === row.id ? (
          <StyledInput
            type="text"
            ...
            onClick={() => handleInputClick("N_Departamento")}
          />
        ) : (
