...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/usuarios/?Usuario=${username}`
      );
      const user = response.data.find(
        (user) => user.Usuario === username && user.Contrasenia === password
      );
      ...
      if (user) {
        ...
        const roleResponse = await axios.get(
          `http://localhost:3000/rolusuario/${user.ID_RolUsuario}`
        );
        ...
        setUserCountries(
          roleData.Paises.split(",").map((paisId) => parseInt(paisId, 10))
        );
  ...
  const handlePasswordChange = async () => {
    ...
    if (!passwordRegex.test(newPassword)) {
      setError(
        "La nueva contraseña debe tener al menos 12 caracteres, una letra mayúscula, un número y un carácter especial."
      );
      return;
    }
    ...
    try {
      ...
      if (updateResponse.status === 200) {
        sessionStorage.setItem("userId", encryptData(userId));
    ...
    <div className="login-container">
      <form onSubmit={handleSubmit} className="login-form">
          ...
          <>
            ...
            <div className="form-group">
              ...
              <input
                ...
                className={`form-control ${
                  isNewPasswordEmpty ? "input-error" : ""
                }`}
                required
              />
            </div>
            <div className="form-group">
              ...
              <input
                ...
                className={`form-control ${
                  isConfirmPasswordEmpty ? "input-error" : ""
                }`}
                required
              />
            </div>
        ...
        <div className="form-floating mb-3 mt-3">
          <a href="/Solicitud" style={{ textDecoration: "none" }}>
            <span className="form-con">He olvidado mi contraseña</span>
          </a>
        </div>
        <br />
      </form>
