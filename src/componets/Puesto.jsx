...
function Puesto() {
    ...
    if (Object.values(newErrors).every((error) => error === "")) {
      try {
        ...
        const nombreExistente = puestos.some(
          (puesto) => puesto.N_Puesto.toLowerCase() === modalValues.N_Puesto.toLowerCase()
        );
  ...
  const handleEditChange = (event, field) => {
    ...
    if (field === "ID_Pais") {
      ...
      const filteredDiv = division.filter((item) => item.ID_Pais === parseInt(value));
      setFilteredDivision(filteredDiv);

      const filteredDep = departamento.filter((item) => item.ID_Pais === parseInt(value));
      setFilteredDepartamento(filteredDep);

      const filteredCC = centrocosto.filter((item) => item.ID_Pais === parseInt(value));
