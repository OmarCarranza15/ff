...
function Puesto() {
  ...
  const SaveModal = async () => {
    ...
    const trimmedNPuesto = modalValues.N_Puesto.trim();
    const newErrors = {
      ID_Pais: "",
      Codigo: "",
      N_Puesto: "",
    ...
    if (Object.values(newErrors).every((error) => error === "")) {
      try {
        ...
        const newPuesto = {
          N_Puesto: trimmedNPuesto,
          Codigo: parseInt(trimmedCodigo, 10),
        ...
        const auditoriaData = {
          Campo_Nuevo: `- País: ${paisResponse.data.N_Pais}¬ - Razón Social: ${rsocialResponse.data.N_RSocial}¬
                       - Division: ${divisionResponse.data.N_Division}¬ - Departamento: ${departamentoResponse.data.N_Departamento}¬ 
                       - Centro de Coste: ${centrocostoResponse.data.Nombre}¬ - Código: ${trimmedCodigo}¬ 
                       - Puesto: ${trimmedNPuesto} `,
          Tabla: 'Puestos',
          Accion: 1, // Inserción
          ID_Usuario: userId,
          N: newIndex
  ...
  const saveChanges = async (id) => {
    const trimmedCodigo = editedRow.Codigo;
    const trimmedNPuesto = editedRow.N_Puesto.trim();
   if (!trimmedNPuesto) {
    ...
    try {
      ...
      if (editedRow && editedRow.Codigo) {
        const codigo = parseInt(trimmedCodigo, 10)
        const response = await axios.get(
          `http://localhost:3000/puesto?Codigo=${trimmedCodigo}`
        );
  
        if (
          response.data.some(
            (puesto) =>
              puesto.Codigo === codigo && puesto.id !== id
          )
        ) {
          ...
          errorNotification.innerHTML = `
            ...
            <span class="error-message">
              El Codigo del Puesto que intenta actualizar ya existe en la base de datos.
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    const editedPuesto = trimmedNPuesto;
      ...
      cell: (row) =>
        editMode && editedRow?.id === row.id ? (
          <StyledInput
            ...
            onClick={() => handleInputClick("N_Puesto")}
          />
        ) : (
