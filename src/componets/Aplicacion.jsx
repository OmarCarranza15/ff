...
function Aplicacion() {
  ...
  const SaveModal = async () => {
    ...
    if (Object.values(newErrors).every((error) => error === "")) {
      try {
        ...
        const auditoriaData = {
          Campo_Nuevo: `- País: ${paisResponse.data.N_Pais}¬ - Aplicacion: ${modalValues.aplicacion}¬ - Ambientes: ${modalValues.Ambientes.map((id) => ambiente.find((a) => a.id === id)?.N_Ambiente)}`, // Usar el índice generado
          Tabla: 'Aplicaciones',
          Accion: 1,
          ID_Usuario: userId,
          N: newIndex
  ...
  const saveChanges = async (id) => {
    if (!editedRow.N_Aplicaciones.trim()) {
