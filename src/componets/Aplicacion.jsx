...
function Aplicacion() {
  ...
  const SaveModal = async () => {
    const trimmedAplicacion = modalValues.aplicacion.trim();
      ...
      try {
        ...
        const aplicacionExists = response.data.some(
          (aplicacion) =>
            ...
            aplicacion.N_Aplicaciones.toLowerCase() === trimmedAplicacion.toLowerCase()
        );
        if (aplicacionExists) {
        ...
        const newAplicacion = {
          N_Aplicaciones: trimmedAplicacion,
        ...
        const auditoriaData = {
          Campo_Nuevo: `- País: ${paisResponse.data.N_Pais}¬ - Aplicacion: ${trimmedAplicacion}¬ - Ambientes: ${modalValues.Ambientes.map((id) => ambiente.find((a) => a.id === id)?.N_Ambiente)}`, // Usar el índice generado
          Tabla: 'Aplicaciones',
          Accion: 1,
          ID_Usuario: userId,
          N: newIndex
  ...
  const saveChanges = async (id) => {
    const trimmedAplicacion = editedRow.N_Aplicaciones.trim();
    if (!trimmedAplicacion) {
      setErrors({ N_Aplicaciones: "El campo Aplicacion no puede estar vacío" });
      showNotification("El campo Aplicacion no puede estar vacío");
      return;
    }
    try {
        const updateRow = {
            ...editedRow,
            N_Aplicaciones: trimmedAplicacion,
            Ambientes: editedRow.Ambientes.join(", "), // Asegúrate de que sea una cadena
        };
        ...
        if (editedRow && editedRow.ID_Pais && editedRow.N_Aplicaciones) {
            ...
            const aplicacionExists = response.data.some((division) => {
                if (division.ID_Pais === parseInt(editedRow.ID_Pais) && division.id !== id) {
                    const editedAplicacionName = trimmedAplicacion ? trimmedAplicacion.toLowerCase() : "";
        ...
        const originalAmbientes = Array.isArray(originalRow.Ambientes) 
            ? originalRow.Ambientes.join(", ") 
            : originalRow.Ambientes || '';
        const editedCountry = editedRow.N_Pais;
        const editedAplicacion = trimmedAplicacion;
      ...
      cell: (row) =>
        editMode && editedRow?.id === row.id ? (
          <StyledInput
            ...
            onClick={() => handleInputClick("N_Aplicaciones")}
          />
        ) : (
      ...
      cell: (row) =>
        editMode && editedRow?.id === row.id ? (
          <Select
            ...
            onClick={() => handleInputClick("Ambientes")}
            menuPosition="fixed"
            menuPlacement="auto"
          />
        ) : (
