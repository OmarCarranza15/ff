...
function Division() {
  ...
  const SaveModal = async () => {
    const trimmedDivision = modalValues.division.trim();
    ...
    } else if (!/^[a-zA-ZñÑáéíóúÁÉÍÓÚ\s]+$/.test(trimmedDivision)) {
      newErrors.division =
      ...
      try {
        ...
        const divisionExists = response.data.some(
          (division) =>
            ...
            division.N_Division.toLowerCase() === trimmedDivision.toLowerCase()
        );
        if (divisionExists) {
        ...
        const auditoriaData = {
          Campo_Nuevo: `- País: ${paisResponse.data.N_Pais}¬ - Razón Social: ${trimmedDivision}`, // Usar el índice generado
          Tabla: "Divisiones",
          Accion: 1,
          ID_Usuario: userId,
          N: newIndex,
  ...
  const saveChanges = async (id) => {
    const trimmedDivision = editedRow.N_Division.trim();
    if (!trimmedDivision) {
    ...
    try {
      ...
      if (editedRow && editedRow.ID_Pais && editedRow.N_Division) {
        ...
        const divisionExists = response.data.some((division) => {
          if (
            ...
            const editedDivisionName = trimmedDivision
              ? trimmedDivision.toLowerCase()
              : "";
      ...
      const editedDivision = trimmedDivision;
  ...
  const columns = [
    ...
    },
    {
      ...
      maxWidth: "450px", // Ajusta el tamaño máximo según sea necesario
      cell: (row) =>
        editMode && editedRow?.id === row.id ? (
          <StyledInput
            type="text"
