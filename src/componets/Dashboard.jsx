...
import { Card as AntCard, Col, Row, Spin, Space } from "antd";
import {
  BarChart,
  Bar,
  XAxis,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const dateRange = generateDateRange(past30Days, today);
        ...
        await Promise.all(
          data.map(async (perfil) => {
            ...
            if (creationDate >= past30Days && creationDate <= today) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        uniquePaises.forEach(pais => {
          const uniquePuestos = [...new Set(data.filter(perfil => perfil.ID_Pais === pais).map(perfil => perfil.ID_Puesto))];
    ...
    <DashboardContainer>
      ...
      <MainContent>
        ...
        <ContentContainer>
          ...
          <Row gutter={[16, 16]}>
            <Col span={24}>
              <Card title="Países" bordered={false}>
                <CountryCardsRow>
                  {loading ? (
                    <LoadingContainer>
                      <Spin tip="Cargando..." />
          ...
          <Row gutter={[16, 16]}>
            <Col span={24}>
              <Card title="Perfiles por País y Aplicaciones" bordered={false}>
                <CountryCardsRow>
                  {loading ? (
                    <LoadingContainer>
                      <Spin tip="Cargando..." />
                    ...
                    countryAppCounts
                      ...
                      .map((countryData) => (
                        <CountryCardCol key={countryData.country}>
                          <CountryCard>
                            <CountryCardTitle>
                              <Space>
                                <Banderas
                                  src={getFlagSrc(countryData.country)}
                                  alt="Flag image"
                                />
                            ...
                            <div
                              style={{
                                height: "400px",
                                overflowY:
                                  countryData.applications.length > 5
                                    ? "auto"
                                    : "hidden",
                              }}
                            >
