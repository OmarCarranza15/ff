...
function RazonSocial() {
  ...
  const SaveModal = async () => {
    const trimmedRSocial = modalValues.rsocial.trim(); 
    ...
    } else if (!/^[a-zA-ZñÑáéíóúÁÉÍÓÚ\s|]+$/.test(trimmedRSocial)) {
      newErrors.rsocial =
      ...
      try {
        ...
        const rsocialExists = response.data.some(
          (rsocial) =>
            ...
            rsocial.N_RSocial.toLowerCase() === trimmedRSocial.toLowerCase()
        );
        if (rsocialExists) {
        ...
        const auditoriaData = {
          Campo_Nuevo: `- País: ${paisResponse.data.N_Pais}¬ - Razón Social: ${trimmedRSocial}`,
...
const saveChanges = async (id) => {
  const trimmedRSocial = editedRow.N_RSocial.trim();
  if (!trimmedRSocial) {
  ...
  try {
      ...
      if (editedRow && editedRow.ID_Pais && editedRow.N_RSocial) {
          ...
          const rsocialExists = response.data.some((rsocial) => {
            if (
              ...
              const editedrsocialName = trimmedRSocial
                ? trimmedRSocial.toLowerCase()
                : "";
      ...
      const editedRSocial = trimmedRSocial;
        ...
        editMode && editedRow?.id === row.id ? (
          <StyledInput
            ...
            onInput={(e) => {
              ...
              handleEditChange(e, "N_RSocial")}
            }
            onChange={(e) => handleEditChange(e, "N_RSocial")}  
            onClick={() => handleInputClick("N_RSocial")}
          />
        ) : (
