...
function MatrizPais() {
  ...
  const SaveModal = async () => {
    const trimmedRol = modalValues.Rol.trim();
    const trimmedPuestoJefe = modalValues.Puesto_Jefe.trim();
    const trimmedTicket = modalValues.Ticket.trim();
    let valid = true;
    const newErrors = {
      ID_Pais: "",
      ID_Puesto: "",
    ...
    if (valid) {
      try {
        ...
        const newPerfil = {
          ...
          Rol: trimmedRol,
          Ticket: parseInt(trimmedTicket, 10),
        ...
        const createCampoNuevo = (modalValues) => {
          let campoNuevo = `- Puesto: ${puestoResponse.data.N_Puesto}¬ - Aplicación: ${aplicacionResponse.data.N_Aplicaciones}¬
                          - Rol: ${trimmedRol}¬ - Jefe Inmediato: ${trimmedPuestoJefe}¬ - Estado: En Servicio ¬`;
          if (modalValues.Ticket) {
            campoNuevo += ` - Ticket: ${trimmedTicket}¬`;
          }
