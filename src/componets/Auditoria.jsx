...
function Auditoria() {
        ...
        const mappedData = await Promise.all(
          data.map(async (auditoria) => {
            ...
            return {
              ...
              createdAt: moment.utc(auditoria.createdAt).tz(timeZone).format('YYYY-MM-DD h:mm:ss A'),
  ...
  const formatDate = (date) => {
    return moment(date).tz(timeZone).format('YYYY-MM-DD h:mm:ss A');
  };
  ...
  const formatDateForFilter = (date) => {
    return moment(date).format('YYYY-MM-DD h:mm:ss A'); // Mantén el formato para el filtro
  };;
