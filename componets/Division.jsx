      ...
      try {
        ...
        const mappedData = await Promise.all(
          data.map(async (division)=> {
            const paisResponse = await axios.get(`http://localhost:3000/pais/${division.ID_Pais}`); 
    ...
    if (Object.values(newErrors).every((error) => error === "")) {
      try {
        ...
        const divisionExists = response.data.some(
          (division) => division.N_Division.toLowerCase() === modalValues.division.toLowerCase()
        );
        if (divisionExists) {
          setErrors({ division: "La Division ya existe" });
          return;
        }
        ...
        const insertResponse = await axios.post(`http://localhost:3000/division`, newDivision);
  ...
  const validateInput = (field, value) => {
    ...
    if (field === "pais") {
      ...
      if (!value.trim()) {
        ...
        newErrors.division = "El campo Division solo acepta letras y espacios en blanco";
      } else {
        newErrors.division = "";
      }
    }
  ...
  return (
    <MainContainer>
      ...
      <ContentContainer>
        ...
        <DataTableContainer>
          <HeaderContainer>
            <Title><h2 className="Title">Divisiones</h2></Title>
            <ButtonGroup>
              <Button primary onClick={toggleFilters}>
                {showFilters ? "Ocultar" : "Buscar"}
              </Button>
              <Button onClick={handleInsert}>Nueva Division</Button>
            </ButtonGroup>
          </HeaderContainer>
...
{showModal && (
        <ModalBackground>
          <ModalWrapper>
            <ModalTitle>Nueva DIvision</ModalTitle>
            <SelectPais
